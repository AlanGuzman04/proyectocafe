...
if (isset($_POST['id'])) {
    ...
    $consulta = mysqli_prepare($mysqli, "SELECT * FROM alumnos WHERE Id = ?");
    ...
    if ($alumno) {
        ...
        $queryInsert = "INSERT INTO alumelimi (IDalume, NoControlA, NombreA, ApePA, ApeMA, ContraA, CorreoA, CarreraA) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($mysqli, $queryInsert);
        mysqli_stmt_bind_param($stmt, "iissssss", $alumno['Id'], $alumno['NoControl'], $alumno['Nombre'], $alumno['ApellidoPA'], $alumno['ApellidoMA'], $alumno['Contraseña'], $alumno['Correo'], $alumno['Carrera']);
        mysqli_stmt_execute($stmt);

        // Eliminar el alumno de la tabla de alumnos
        $queryDelete = "DELETE FROM alumnos WHERE Id = ?";
