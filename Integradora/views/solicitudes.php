...
<body>
    ...
    <div class="container mt-3">
        ...
        <table class="table table-striped">
            ...
            <tbody>
            <?php
            $pedidos = mysqli_query($mysqli, "SELECT * FROM nuevo_alumnos");
            while ($res = mysqli_fetch_assoc($pedidos)) {
                ?>
                <tr>
                    <form action = '' method='POST' id='frmAceptar'>
                    <td> <?php echo $res['Id']; ?> </td>
                    <td> <?php echo $res['NocontrolN']; ?> <input type = 'hidden' value = '<?php echo $res['NocontrolN']; ?>' name ='id'></td>
                    <td> <?php echo $res['NombreN']; ?> </td>
                    <td> <?php echo $res['ApellidopaN']; ?> </td>
                    <td> <?php echo $res['ApellidomaN']; ?> </td>
                    <td> <?php echo $res['CarreraN']; ?> </td>
                    <td> <?php echo $res['CorreoN']; ?> </td>
                    
                    <td>
                        <a  href="#" class="btn btn-outline-primary" onClick="Aceptar(<?php echo $res['NocontrolN']; ?>)">Aceptar</a>
                        <a  href="#" class="btn btn-outline-danger" onClick="Eliminar(<?php echo $res['NocontrolN']; ?>)">Rechazar</a>
                    </td>
                    </form>
    ...
    <script>
        ...
        function Aceptar(id){
            if(confirm('Seguro que quieres Aceptar al Alumno')){
                alert("Aceptar :" + id);
                form.action = "../controllers/aceptaralum.php";
                form.submit();
            }
        }
        function Eliminar(id){
            if(confirm('Seguro que quieres Eliminar al Alumno')){
                alert("Eliminar :" + id);
                form.action = "../controllers/rechazaralum.php";
                form.submit();
            }
        }
    </script>
