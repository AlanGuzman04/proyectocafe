...
<body>
    ...
    <div class="container mt-3">
        ...
        <table class="table table-striped">
            ...
            <tbody>
            <?php
            $pedidos = mysqli_query($mysqli, "SELECT * FROM quejas");
            while ($res = mysqli_fetch_assoc($pedidos)) {
                ?>
                <tr>
                    <form action = '' method='POST' id='frmAceptar'>
                    <td> <?php echo $res['IDqueja']; ?> <input type = 'hidden' value = '<?php echo $res['IDqueja']; ?>' name ='id'></td>
                    <td> <?php echo $res['NoControlAlum']; ?> </td>
                    <td> <?php echo $res['NombreALum']; ?> </td>
                    <td> <?php echo $res['queja']; ?> </td>

                    <td>
                        
                        <a  href="#" class="btn btn-outline-danger" onClick="Eliminar(<?php echo $res['IDqueja']; ?>)">Eliminar</a>
                    </td>
                    </form>
    ...
    <script>
        ...
        function Eliminar(id){
            if(confirm('Seguro que quieres Eliminar al Alumno')){
                alert("Eliminar :" + id);
                form.action = "../controllers/eliminarqueja.php";
                form.submit();
            }
        }
    </script>
